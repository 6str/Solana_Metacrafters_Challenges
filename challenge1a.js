...
const mainFunction = async () => {
    var args = process.argv.slice(2);   // remove first two args: node path & script path
    const pubKeyArg = args[0]
    await getWalletBalance(pubKeyArg);
    await airDropSol(pubKeyArg);
    await getWalletBalance(pubKeyArg);
}
